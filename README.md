<div align="center">
	<img width="100%" src="https://capsule-render.vercel.app/api?type=waving&color=gradient&customColorList=6,11,20&height=200&section=header&text=Anchal%20Kumari&fontSize=50&fontColor=fff&animation=twinkling&fontAlignY=35&desc=✨%20AI/ML%20%20%7C%20Full-Stack%20Developer%20%7C%20CSE%20Student%20%7C%20Problem%20Solver%20✨&descAlignY=55&descSize=20" />
</div>

<div align="center">
	<img src="https://readme-typing-svg.herokuapp.com?font=Orbitron&size=32&duration=3000&pause=1000&color=FF6B6B&center=true&vCenter=true&multiline=true&width=800&height=80&lines=🚀+AI/ML+%26+Full-Stack+Developer+🚀;🎓+CSE+Student+@+KIIT+University+🎓;🏆+Hackathon+Enthusiast+%7C+Project+Builder+🏆;✨+Let%27s+Innovate+Together!+✨" alt="Typing SVG" />
</div>


---

## 🎭 About Me

<img align="right" alt="Anchal Kumari" width="170" src="https://avatars.githubusercontent.com/u/183522268" />

- 👩‍💻 **CSE Student** @ Kalinga Institute of Industrial Technology (2023-2027)
- 🤖 **AI/ML, Web Development, Product Analytics**
- 🏆 **Hackathon Participant & Project Builder**
- 💻 **Stack:** C, Java, Python, SQL, JavaScript, React, Node.js, Power BI
- 🌱 **Learning:** Machine Learning, Data Visualization, Cloud
- ✨ **Skills:** Problem Solving, Team Collaboration, Communication
- 📧 **Contact:** kumarianchal.1970@gmail.com | +91-9931591586
- 🌐 **LinkedIn:** [Anchal Kumari](https://www.linkedin.com/in/anchal-kumari-9aa082286)

<div align="center">
	<img src="https://komarev.com/ghpvc/?username=anchallll456&label=Profile%20Views&color=ff69b4&style=for-the-badge&logo=eye" alt="profile views" />
	<img src="https://img.shields.io/github/followers/anchallll456?label=Followers&style=for-the-badge&color=orange&logo=github" alt="followers" />
	<img src="https://img.shields.io/badge/KIIT%20University-CSE%20'27-brightgreen?style=for-the-badge&logo=graduation-cap" alt="education" />
	<img src="https://img.shields.io/badge/CGPA-9.09-blue?style=for-the-badge&logo=academic" alt="cgpa" />
</div>

---

## 🛠️ My Tech Toolkit

<div align="center">
	<img src="https://user-images.githubusercontent.com/74038190/212257454-16e3712e-945a-4ca2-b238-408ad0bf87e6.gif" width="100"><img src="https://user-images.githubusercontent.com/74038190/212257472-08e52665-c503-4bd9-aa20-f5a4dae769b5.gif" width="100">
	<img src="https://readme-typing-svg.herokuapp.com?font=Orbitron&size=25&duration=3000&pause=1000&color=FFD700&center=true&vCenter=true&width=600&lines=Crafting+AI+Solutions;With+These+Amazing+Tools!" alt="Tools" />
</div>

### 💻 **Programming Languages**
<div align="center">
	<img src="https://skillicons.dev/icons?i=python,java,c,cpp,js,sql&theme=dark" alt="Programming Languages" />
</div>

## 🛠️ Skills

- **Languages:** C, Java, Python, SQL, JavaScript, HTML, CSS
- **Frameworks/Tools:** React, Node.js, Power BI, MS Excel, DBMS, Web Development
- **Other:** Data Cleaning, Data Visualization, Web Scraping, Machine Learning
- **Soft Skills:** Problem Solving, Team Collaboration, Communication

---

## 🎓 Education

- **Kalinga Institute of Industrial Technology, Bhubaneswar**  
	B.Tech, Computer Science Engineering (2023–2027)  
	CGPA: 9.09
---

## 🚀 Projects

- **MindMate – AI-Powered Mental Health Assistant**  
	Multi-modal AI assistant with text, voice, and avatar interactions. Features emotion detection, mood tracking, and journaling using TensorFlow, Hugging Face, and Firebase.
- **Women Safety Analytics – Protecting Women from Safety Threats**  
	Real-time smart surveillance system for women’s safety, using OpenCV, TensorFlow, PyTorch, and data visualization.
- **Agricultural E-Commerce Platform: Architecture & AI Integration**  
	Farmer-to-customer marketplace with authentication, real-time product listing, and AI-based search.
- **Customer Churn Prediction for OTT Platforms**  
	Machine learning model using Python and Scikit-learn to predict customer churn and analyze user behavior.
- **Customer Personality Analysis Dashboard**  
	Power BI dashboard to analyze customer demographics and support data-driven marketing.

---

## 🏆 Achievements

- Asia’s 5th-ranked MUN hosted by KIIT (2024)
- National Hackathon – India (2024 & 2025)
- Presented Mindmate at ICDDCIT 2025, KIIT University
- 1st prize for Mindmate at TechFrontier, KIDP2DS Society, KIIT (2025)
- Igniting India’s Electric Revolution Maruti ePulse (2025)
- Presented Fesal Seva Project at ICDDCIT 2026, KIIT University
- Contributing artist for Youth Red Cross Society, KIIT (Since 2024)

---

## 🎨 Interests

Photography & editing, Video editing, Reading, Graphic Designing

